...
% open file
file_name = sprintf('rasters_syntax_%s_%s_%s_%s_blocks_%s_sort_%i.html', settings.patient, settings.run_name, settings.comparison_name, settings.lock_to_word, settings.blocks_str, settings.sort_sentences_according_length);
...
% ERPs
curr_fig_name = sprintf('ERP0_%s_%s_%s_blocks_%s_%s.png', settings.patient, settings.run_name, settings.comparison_name, settings.blocks_str, settings.lock_to_word);
curr_fig_name = fullfile('Figures', 'ERP', curr_fig_name);
fprintf(fileID, '<img class="right" src="%s" style="width:1024px;height:512px;">\n', curr_fig_name);
fprintf(fileID, '<br>\n');
curr_fig_name = sprintf('ERP1_%s_%s_%s_blocks_%s_%s.png', settings.patient, settings.run_name, settings.comparison_name, settings.blocks_str, settings.lock_to_word);
curr_fig_name = fullfile('Figures', 'ERP', curr_fig_name);
fprintf(fileID, '<img class="right" src="%s" style="width:1024px;height:512x;">\n', curr_fig_name);
fprintf(fileID, '<br>\n');

% GAT
curr_fig_name = sprintf('gat_%s_%s_%s_blocks_%s_%s.png', settings.patient, settings.run_name, settings.comparison_name, settings.blocks_str, settings.lock_to_word);
curr_fig_name = fullfile('Figures', 'GAT', curr_fig_name);
fprintf(fileID, '<img class="right" src="%s" style="width:512px;height:256px;">\n', curr_fig_name);
curr_fig_name = sprintf('gat_diag_%s_%s_%s_blocks_%s_%s.png', settings.patient, settings.run_name, settings.comparison_name, settings.blocks_str, settings.lock_to_word);
curr_fig_name = fullfile('Figures', 'GAT', curr_fig_name);
fprintf(fileID, '<img class="right" src="%s" style="width:512px;height:256px;">\n', curr_fig_name);
fprintf(fileID, '<br>\n');
...
curr_fig_name = ['pca_sentences_' file_name '.png'];
curr_fig_name = fullfile('Figures', 'PCA', curr_fig_name);
fprintf(fileID, '<img class="right" src="%s" style="width:1024px;height:512x;">\n', curr_fig_name);
fprintf(fileID, '<br>\n');
...
switch settings.patient
    ...
    case {'En_01', 'En_02', 'patient_479'}
        script_location = fullfile('..', '..', 'Data', settings.hospital, settings.patient, 'clusters_electrode_montage.m');
