function [rasters, trials_info] = generate_rasters_from_spike_trains(trials_info, settings)

if settings.sort_sentences_according_length
    words = regexp(trials_info.stimuli{1}, '\s+', 'start');  % find spaces (which separate words)
    wordsPerSentence = 1 + cellfun(@length, words, 'UniformOutput', true);  %count words
    [wordsPerSentence, IX_word_length] = sort(wordsPerSentence, 'descend');
    trials_info.stimuli{1} = trials_info.stimuli{1}(IX_word_length);
    trials_info.wordsPerSentence = wordsPerSentence;
end
...
for unit = 1:length(all_units)
    ...
    if settings.generate_condition_rasters
            ...
            for cnd = 1:length(trials_info.trial_times_per_condition)
                ...
                if settings.sort_sentences_according_length                    
                    curr_raster = curr_raster(IX_word_length, :);
                    spike_times_all_trials = spike_times_all_trials(IX_word_length);
                end
                rasters.(unit_name).(cnd_name).matrix = curr_raster;
                rasters.(unit_name).(cnd_name).spike_times = spike_times_all_trials;
                raster_all_trials = [raster_all_trials; curr_raster];
            end
            rasters.(unit_name).All__trials.matrix = raster_all_trials;
...
if strcmp(settings.run_name, 'sentences')
    settings_fields = {'patient', 'comparison_name', 'run_name', 'lock_to_word'};
else
    settings_fields = {'patient', 'comparison_name', 'run_name'};
end
