function figure_names = generate_raster_figures_singles(rasters, trials_info, settings)
...
if ~strcmp(comparison_name, 'All__trials')
    all_conditions = setdiff(all_conditions, 'All__trials');
end
...
for u = 1:length(all_units)
    unit_name = all_units{u};
            for cnd = 1:num_conditions
                ...
                if strcmp(settings.run_name, 'sentences')
                    settings_fields = {'patient', 'unit_name', 'comparison_name', 'run_name', 'blocks_str', 'lock_to_word', 'sort_sentences_according_length'};
                else
                    settings_fields = {'patient', 'unit_name', 'comparison_name', 'run_name', 'block_type'};
                end
                ...
                % Generate raster plot
                    ...
                    if settings.generate_condition_rasters && generate_raster_plots
                        f = figure('color', [1 1 1], 'visible', 'off','Units', 'Normalized', 'OuterPosition', [0.2, 0.2, 0.4, 0.8]);
                        % Load raster data
...
%                         set(gcf, 'visible', 'off')
                        ...
                        set(gca, 'FontSize', 8)
                        if settings.sort_sentences_according_length
                            set(gca, 'ytick', 1:15:length(trials_info.wordsPerSentence), 'yticklabel', trials_info.wordsPerSentence(1:15:end))
                            ylabel('Number of words in trial', 'fontsize', 10)
                        else
			...
			%if exist(fullfile('../../Figures/Rasters', [file_name '.png']), 'file') ~= 2 
...
file_name = sprintf('figure_names_%s_%s.mat', settings.patient, settings.run_name, settings.comparison_name);
save(fullfile('..', '..', 'Output', file_name), 'figure_names')
    ...
    if settings.generate_key_press_rasters
        ...
        settings.unit_name = unit_name;
        settings_fields = {'block_type', 'patient', 'unit_name', 'run_name'};
        params = []; params_fields = [];
        file_name = get_file_name_curr_run(settings, params, settings_fields, params_fields);
        file_name = ['raster_button_press_' file_name];
%         figure_names{u+1, 1} =  [file_name '.png'];
        saveas(f, fullfile('../../Figures/Rasters', [file_name '.png']), 'png')
        close(f)  
    end
end
