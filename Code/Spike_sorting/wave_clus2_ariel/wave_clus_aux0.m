function varargout = wave_clus_aux0(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @wave_clus_aux0_OpeningFcn, ...
                   'gui_OutputFcn',  @wave_clus_aux0_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function wave_clus_aux0_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = wave_clus_aux0_OutputFcn(hObject, eventdata, handles)
...
for i=4:8
    if (isfield(par, sprintf('nbins%d', i)))
        set(handles.(sprintf('isi%d_nbins', i)),'string',par.(sprintf('nbins%d', i)));
        set(handles.(sprintf('isi%d_bin_step', i)),'string',par.(sprintf('bin_step%d', i)));
    end
end
...
for i=4:8
    if (~isempty(USER_DATA{9+i}))   % check if cluster is empty
        set(handles.(['fix' num2str(i) '_button']), 'value', ...
                          par.(['fix' num2str(i)]));
    else
        % empty cluster cannot be fixed:
        set(handles.(['fix' num2str(i) '_button']), 'value', 0);
    end
end
USER_DATA{1} = par;
set(handles.wave_clus_aux0,'userdata',USER_DATA)
set(h_fig,'userdata',USER_DATA)
set(h_fig1,'userdata',USER_DATA)

plot_spikes_aux(handles, 0);
...
USER_DATA = get(handles.wave_clus_aux0,'userdata');
par = USER_DATA{1};
...
aux_reject_cluster(4, 0, handles);
...
aux_reject_cluster(5, 0, handles);
...
aux_reject_cluster(6, 0, handles);
...
aux_reject_cluster(7, 0, handles);
...
aux_reject_cluster(8, 0, handles);
