...
%handles.par.system = 'windows';
handles.par.system = 'linux';
...
handles.par.fmin = 300;                     %high pass filter
handles.par.fmax = 3000;                    %low pass filter
...
for k=1:length(files)
    ...
    [b,a]=ellip(2,0.1,40,[handles.par.fmin handles.par.fmax]*2/(handles.par.sr));
    xf=filtfilt(b,a,x);
    ...
    1:handles.par.num_temp,tree(1:handles.par.num_temp,5:size(tree,2)),[temp temp],[0 tree(1,5)],'k:')
    subplot(3,5,6)
    hold on
...
% Rescale spike's axis 
    ...
    title([pwd '/' char(file_to_cluster)],'Fontsize',14)    
    toc
    if print2file==0;
        print
    else
