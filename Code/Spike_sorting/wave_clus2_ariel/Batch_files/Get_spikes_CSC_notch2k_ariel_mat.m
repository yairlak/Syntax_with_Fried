function Get_spikes_CSC_notch2k_ariel_mat(channels, param_set_name, save_filt_data);
...
%                      changed from 2 to 4.
% Modified: 10.08.2015 (Ariel). check_bad_freq.
...
if (nargin < 3)
    save_filt_data = false;    % save also the filtered signal in a separate file.
end
...
switch (param_set_name)
  ...
  case 'TASMC2'
    handles = TASMC2_default_params();

  case 'UCLA'
    handles = UCLA_default_params();

  case 'UCLA_Neuralynx'
    handles = UCLA_Neuralynx_default_params();
...
handles.par.param_set_name = param_set_name;   % for saving param_set_name
                                                          % as well.
...
recording_len_min = (timeend - time0) ./ (1E6 .* 60);
%% SECTION TITLE
% DESCRIPTIVE TEXT
if (recording_len_min > handles.par.min_rec_for_segmentation_min)
    handles.par.segments = ceil(recording_len_min ./ ...
                                (handles.par.segments_length));
else
    % no splitting into segments - use the whole recording as 1 chunk:
    handles.par.segments = 1;
end

if (exist(['.', filesep, 'break_events.mat']))
    load(['.', filesep, 'break_events.mat']); 
else
    break_ann_behavior = [];
    break_onset_time_sec = [];
end
...
for k=1:length(channels)
    ...
    [tsmin_ind, tsmax_ind, tsmin, tsmax] = ...
...
%        [recmin(j), recmax(j), length(data)]
        ...
        [spikes,thr,index] = amp_detect(x,handles.par,ts_ind, ...
                                        save_filt_data, channel);       %detection with amp. thresh.
