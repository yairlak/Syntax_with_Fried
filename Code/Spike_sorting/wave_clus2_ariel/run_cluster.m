...
%        !cluster data.run
    case {'GLNX86', 'GLNXA64'}
     % ARIEL: comment out 3 lines (if, copyfile, end)
%        if exist([pwd '/cluster_linux.exe'])==0
%            directory = which('cluster_linux.exe');
%            copyfile(directory,pwd);
%        end
%        unix(sprintf('./cluster_linux.exe %s.run',fname));
     % ARIEL:
        directory = which('cluster_linux.exe');
        if (isempty(directory))
            error(['Unable to find cluster_linux.exe, probably not in search ' ...
                   'path\nor has no run permission.']);
        end
        fprintf('%s %s.run', directory, fname) ;
        unix(sprintf('%s %s.run', directory, fname));
end

clu=load([fname '.dg_01.lab']);
tree=load([fname '.dg_01']); 
delete(sprintf('%s.run',fname));
delete *.mag *.edges *.param
delete(fname);
