    ...
    for channel in channels:
        ...
        for band, fmin, fmax in params.iter_freqs:
            ...
            if any(["WORDS_ON_TIMES" in s for s in event_ids_epochs]):
                ...
                power, power_ave, _ = analyses_single_unit.average_high_gamma(epochs_resampled, curr_event_id_to_plot, band, fmin, fmax, params.freq_step, False, params)
                ...
                analyses_single_unit.plot_and_save_high_gamma(power, power_ave, event_str, log_all_blocks, file_name,
            ...
            for event_str in ["FIRST_WORD", "LAST_WORD", "KEY"]:
                if any([event_str in s for s in event_ids_epochs]):
                    ...
                    if event_str == "FIRST_WORD":  # Calculate baseline when alignment is locking to first word.
                        power, power_ave, baseline = analyses_single_unit.average_high_gamma(epochs_resampled, curr_event_id_to_plot, band,
                                                                                             fmin, fmax, params.freq_step, None, params)
                    else:
                        if event_str == "KEY":  # Calculate baseline when alignment is locking to first word.
                            power, power_ave, _ = analyses_single_unit.average_high_gamma(epochs_resampled, curr_event_id_to_plot, band, fmin, fmax, params.freq_step, None, params)
                        else:
                            power, power_ave, _ = analyses_single_unit.average_high_gamma(epochs_resampled, curr_event_id_to_plot, band,
