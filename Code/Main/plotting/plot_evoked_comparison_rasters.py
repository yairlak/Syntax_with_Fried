...
parser.add_argument('-field-for-ylabels', default='phone_string', help='Field from metadata to use for yticklabels')
...
hgt = int(len(args.queries_to_compare)*5)
plt.rcParams["figure.figsize"] = [10, hgt]
...
for i_cluster, cluster_name in enumerate(epochs_spikes.info['ch_names']):
    ...
    #condition_names = []
    colors_dict = {}
    for q, (condition_name, curr_query, color) in enumerate(args.queries_to_compare):
        ...
        fig_raster = curr_condition_epochs.plot_image(0, order=IX, vmin=0, vmax=1, cmap='binary', colorbar=False, show=False)
