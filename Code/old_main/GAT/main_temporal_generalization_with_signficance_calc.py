...
for comparison, events in enumerate(mat['custom_events_all_comparisons'][0]):
    ...
    if len(event_id) > 1:
        ...
        #gat = GeneralizationAcrossTime(clf=LinearSVC(), train_times=train_times, test_times=test_times, predict_mode='cross-validation', n_jobs=30)
        ...
        # Load permutation histograms for significance testing
#        output_file_name = 'Hists_Cluster_Length_' + curr_patient + '_' + curr_block + '_' + curr_comparison_name + '_' + lock_to_word[0][comparison][0] + '.pkl'
        ...
        with open(output_file_name, 'rb') as input:
            cluster_length_hists = pickle.load(input)

        cumsum = np.cumsum(cluster_length_hists)
        ...
        for i in range(len(IX)):
            if IX[i] == 1:
                ...
                if in_cluster:
                    cluster_start_IXs.append(i - curr_cluster_size)
                    cluster_sizes.append(curr_cluster_size)
                    # Check significance
                    p = (cumsum[-1] - cumsum[curr_cluster_size+1])/cumsum[-1]
                    p_values.append(p)
                # Reset size counter for next cluster
                curr_cluster_size = 0
                in_cluster = False
...
#        fig_file_name = 'gat_' + curr_patient + '_' + curr_block + '_' + curr_comparison_name + '_' + lock_to_word[0][comparison][0] + '.png'
        fig_file_name = 'gat_' + curr_patient + '_' + curr_block + '_' + curr_comparison_name + '_' + 'last' + '.png'
        fig_gat.savefig(os.path.join('..', '..', 'Figures', 'figures_GAT', fig_file_name))
        plt.close(fig_gat)
        ...
        for i in range(len(cluster_start_IXs)):
            if p_values[i] < 0.05/15:
                st = cluster_start_IXs[i]
                st = gat.train_times_['times'][st]
                ed = cluster_start_IXs[i] + cluster_sizes[i] - 1
                ed = gat.train_times_['times'][ed]
                plt.axvspan(st, ed, facecolor='green', alpha=0.5)

#        fig_diag_file_name = 'gat_diag_' + curr_patient + '_' + curr_block + '_' + curr_comparison_name + '_' + lock_to_word[0][comparison][0] + '.png'
        fig_diag_file_name = 'gat_diag_' + curr_patient + '_' + curr_block + '_' + curr_comparison_name + '_' + 'last' + '.png'
        fig_diag.savefig(os.path.join('..', '..', 'Figures', 'figures_GAT', fig_diag_file_name))
        plt.close(fig_diag)
